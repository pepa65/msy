...
fn run_sy(args: &[&str]) -> std::process::Output {
	Command::new(env!("CARGO_BIN_EXE_msy")).args(args).output().expect("Failed to execute sy")
}
...
fn test_sync_overwrites_existing_symlink() {
	...
	assert_eq!(dest_link_after.to_str().unwrap(), "target.txt", "Symlink should be overwritten to point to target.txt");
}
...
fn test_sync_symlink_over_regular_file() {
	...
	assert!(dest.join("link.txt").is_symlink(), "After sync, link.txt should be a symlink");
}
...
fn test_sync_multiple_symlinks_same_target() {
	...
	assert!(output.status.success(), "Sync should succeed. stderr: {}", String::from_utf8_lossy(&output.stderr));
...
fn test_sync_symlink_to_empty_dest() {
	...
	assert!(dest.join("link.txt").is_symlink(), "link.txt should be a symlink");
...
fn test_sync_skips_identical_symlink() {
	...
	assert!(
		stdout.contains("updated:") && stdout.contains("0") || stdout.contains("Files updated:     0"),
