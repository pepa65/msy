...
fn test_min_size_filters_small_files() {
	...
	let output = Command::new(sy_bin())
		.args(["--min-size", "1KB", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --min-size failed: {}", String::from_utf8_lossy(&output.stderr));

	// Only medium.txt should be synced
	assert!(!dest.path().join("tiny.txt").exists(), "tiny.txt should be filtered out by --min-size 1KB");
	assert!(!dest.path().join("small.txt").exists(), "small.txt should be filtered out by --min-size 1KB");
	assert!(dest.path().join("medium.txt").exists(), "medium.txt should be synced (>= 1KB)");
}
...
fn test_min_size_exact_boundary() {
	...
	assert!(dest.path().join("exact.txt").exists(), "exact.txt (exactly 1KB) should be synced");
	assert!(!dest.path().join("under.txt").exists(), "under.txt (1023 bytes) should be filtered out");
}
...
fn test_max_size_filters_large_files() {
	...
	// Only sync files <= 1KB
	let output = Command::new(sy_bin())
		.args(["--max-size", "1KB", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --max-size failed: {}", String::from_utf8_lossy(&output.stderr));

	// Only small.txt should be synced
	assert!(dest.path().join("small.txt").exists(), "small.txt should be synced (<= 1KB)");
	assert!(!dest.path().join("medium.txt").exists(), "medium.txt should be filtered out by --max-size 1KB");
	assert!(!dest.path().join("large.txt").exists(), "large.txt should be filtered out by --max-size 1KB");
}
...
fn test_max_size_exact_boundary() {
	...
	// Exact boundary should be included
	assert!(dest.path().join("exact.txt").exists(), "exact.txt (exactly 1KB) should be synced");
	assert!(!dest.path().join("over.txt").exists(), "over.txt (1025 bytes) should be filtered out");
}
...
fn test_min_max_size_combined() {
	...
	let output = Command::new(sy_bin())
		.args(["--min-size", "500", "--max-size", "2KB", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --min-size --max-size failed: {}", String::from_utf8_lossy(&output.stderr));

	// Only small and medium should be synced
	assert!(!dest.path().join("tiny.txt").exists(), "tiny.txt should be filtered (< 500)");
	assert!(dest.path().join("small.txt").exists(), "small.txt should be synced (500-2KB)");
	assert!(dest.path().join("medium.txt").exists(), "medium.txt should be synced (500-2KB)");
	assert!(!dest.path().join("large.txt").exists(), "large.txt should be filtered (> 2KB)");
}
...
fn test_min_size_greater_than_max_size_fails() {
	...
	// Should fail: min > max
	let output = Command::new(sy_bin())
		.args(["--min-size", "10KB", "--max-size", "1KB", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(!output.status.success(), "Should fail when --min-size > --max-size");
...
fn test_size_human_readable_formats() {
	...
	for (size_str, should_exist) in formats {
		...
		let output = Command::new(sy_bin())
			.args(["--min-size", size_str, &format!("{}/", source.path().display()), dest_test.path().to_str().unwrap()])
			.output()
			.unwrap();

		assert!(output.status.success(), "sy --min-size {} failed: {}", size_str, String::from_utf8_lossy(&output.stderr));
...
fn test_min_size_zero_byte_files() {
	...
	let output = Command::new(sy_bin())
		.args(["--min-size", "1", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success());

	assert!(!dest.path().join("empty.txt").exists(), "Empty file should be filtered by --min-size 1");
...
fn test_size_filter_with_directories() {
	...
	assert!(dest.path().join("subdir").exists(), "Directories should be synced regardless of --min-size");
