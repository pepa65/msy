...
fn test_archive_includes_git_directory() {
	...
	assert!(output.status.success(), "sy -a failed: {}", String::from_utf8_lossy(&output.stderr));

	// Archive mode should include .git
	assert!(dest.path().join(".git").exists(), ".git directory should be included with -a flag");
	assert!(dest.path().join(".git/config").exists(), ".git/config should be included with -a flag");
...
fn test_archive_includes_hidden_files() {
	...
	assert!(dest.path().join(".hidden").exists(), ".hidden should be included with -a");
	assert!(dest.path().join(".config").exists(), ".config should be included with -a");
...
fn test_archive_preserves_permissions() {
	...
	assert_eq!(dest_perms.mode() & 0o777, 0o755, "Permissions should be preserved with -a");
}
...
fn test_archive_syncs_gitignored_files() {
	...
	assert!(dest.path().join("app.log").exists(), "gitignored files should be synced with -a");
	assert!(dest.path().join("file.txt").exists());
...
fn test_default_includes_git() {
	...
	assert!(dest.path().join(".git").exists(), ".git should be included by default");
	assert!(dest.path().join("file.txt").exists());
}
...
fn test_exclude_vcs_excludes_git() {
	...
	let output = Command::new(sy_bin())
		.args(["--exclude-vcs", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --exclude-vcs failed: {}", String::from_utf8_lossy(&output.stderr));

	// .git should be excluded with --exclude-vcs
	assert!(!dest.path().join(".git").exists(), ".git should be excluded with --exclude-vcs");
	assert!(dest.path().join("file.txt").exists());
}
...
fn test_default_syncs_gitignored_files() {
	...
	assert!(dest.path().join("debug.log").exists(), "*.log should be synced by default");
	assert!(dest.path().join("build").exists(), "build/ should be synced by default");
...
fn test_gitignore_flag_respects_gitignore() {
	...
	let output = Command::new(sy_bin())
		.args(["--gitignore", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --gitignore failed: {}", String::from_utf8_lossy(&output.stderr));

	// .log files should NOT be synced with --gitignore
	assert!(!dest.path().join("debug.log").exists(), "*.log should be excluded with --gitignore");
	assert!(dest.path().join("file.txt").exists());
}
...
fn test_archive_is_complete_backup() {
	...
	assert!(dest.path().join(".gitignore").exists(), ".gitignore with -a");
	assert!(dest.path().join(".hidden").exists(), ".hidden with -a");
	assert!(dest.path().join("temp.tmp").exists(), "ignored file with -a");
	assert!(dest.path().join("file.txt").exists());
...
fn test_gitignore_without_exclude_vcs() {
	...
	assert!(!dest.path().join("debug.log").exists(), "gitignore rules should apply with --gitignore");
}
...
fn test_developer_workflow_flags() {
	...
	let output = Command::new(sy_bin())
		.args(["--gitignore", "--exclude-vcs", &format!("{}/", source.path().display()), dest.path().to_str().unwrap()])
		.output()
		.unwrap();

	assert!(output.status.success(), "sy --gitignore --exclude-vcs failed: {}", String::from_utf8_lossy(&output.stderr));
	...
	assert!(!dest.path().join("debug.log").exists(), "*.log should be excluded with --gitignore");
	assert!(!dest.path().join("build").exists(), "build/ should be excluded with --gitignore");
