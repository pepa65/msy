...
impl S3Transport {
	...
	pub async fn new(bucket: String, prefix: String, region: Option<String>, endpoint: Option<String>) -> Result<Self> {
		...
		let store = Arc::new(builder.build().map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to create S3 client: {}", e))))?);
...
impl Transport for S3Transport {
	...
	async fn scan(&self, _path: &Path) -> Result<Vec<FileEntry>> {
		...
		while let Some(meta) = list_stream.next().await {
			let meta = meta.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to retrieve object metadata: {}", e))))?;
	...
	async fn scan_streaming(&self, _path: &Path) -> Result<BoxStream<'static, Result<FileEntry>>> {
		...
		let mapped = stream.map(move |meta_res| {
			let meta = meta_res.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to retrieve object metadata: {}", e))))?;
	...
	async fn metadata(&self, _path: &Path) -> Result<std::fs::Metadata> {
		...
		Err(SyncError::Io(std::io::Error::other("metadata() not supported for S3, use file_info() instead")))
	}
	...
	async fn file_info(&self, path: &Path) -> Result<FileInfo> {
		...
		let meta = self
			...
			.head(&object_path)
			.await
			.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to get S3 object metadata: {}", e))))?;
	...
	async fn create_dir_all(&self, path: &Path) -> Result<()> {
		...
		self.store
			...
			.await
			.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to create S3 directory marker: {}", e))))?;
	...
	async fn copy_file(&self, source: &Path, dest: &Path) -> Result<TransferResult> {
		...
		if size < MULTIPART_THRESHOLD {
			...
			self.store
				...
				.await
				.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to upload to S3: {}", e))))?;
		} else {
	...
	async fn remove(&self, path: &Path, _is_dir: bool) -> Result<()> {
		...
		self.store
			.delete(&object_path)
			.await
			.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to delete S3 object: {}", e))))?;

		Ok(())
	}

	async fn create_hardlink(&self, _source: &Path, _dest: &Path) -> Result<()> {
		Err(SyncError::Io(std::io::Error::other("Hardlinks not supported on S3")))
	}

	async fn create_symlink(&self, _target: &Path, _dest: &Path) -> Result<()> {
		Err(SyncError::Io(std::io::Error::other("Symlinks not supported on S3")))
	}
	...
	async fn read_file(&self, path: &Path) -> Result<Vec<u8>> {
		...
		let result = self
			...
			.get(&object_path)
			.await
			.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to download from S3: {}", e))))?;

		let bytes = result
			.bytes()
			.await
			.map_err(|e| SyncError::Io(std::io::Error::other(format!("Failed to read S3 object body: {}", e))))?;
