...
fn bench_delta_sync_vs_full_copy(c: &mut Criterion) {
	...
	Command::new("git").args(["init"]).current_dir(source_delta.path()).output().unwrap();
	...
	Command::new("git").args(["init"]).current_dir(source_full.path()).output().unwrap();
...
criterion_group!(benches, bench_delta_sync_small_change, bench_delta_sync_vs_full_copy, bench_delta_sync_large_file);
